...
public class PageController {
    ...
    public PageController(AssetRepository assetRepository,
    ...
    private List<DepreciationBar> buildDepreciationBars() {
        ...
        for (int month = 1; month <= 12; month++) {
            chargesByMonth.put(LocalDate.of(Year.now().getValue(), month, 1).format(DateTimeFormatter.ofPattern("MMM")), BigDecimal.ZERO);
        }
        for (DepreciationPosting posting : postings) {
            ...
            String label = LocalDate.of(Year.now().getValue(), month, 1).format(DateTimeFormatter.ofPattern("MMM"));
